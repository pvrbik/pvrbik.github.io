...
k := k+1;
   U := Phi(gam*(Quo(U,lcoeff(U,x),t, 'q1') mod q),p^k,q);
   W := Phi(gam*(Quo(W,lcoeff(W,x),t, 'q2') mod q),p^k,q);
od;
...
# Error, invalid input: Phi expects its 1st argument, a, to be of type polynom, but received (-t^3+t^5)/(1+t^4-t^2)+(-t^7-t^2-t^6+t^4)*x/(1+t^4-t^2)+(t-t^6+t^2-t^4+t^3)*x^2/(1+t^4-t^2)+(-t+t^4-t^3)*x^3/(1+t^4-t^2)+(t+t^2+t^3+t^5-t^4)*x^4/(1+t^4-t^2)+(t^3-t^4-t^2)*x^5/(1+t^4-t^2)
q1; q2;
...
Rem( subs(t=RootOf(p),c*ss), subs(t=RootOf(p),u0), x, 'qq') mod q;
...
Monic(qq, p, q);
# Error, invalid input: Monic expects its 1st argument, a, to be of type polynom, but received (-t^3+t^2+1)/(1+t+t^2)+(1-t^2-t)*x/(-1+t)-t*x^2
W := -t*x+x+1-x*t^2-x^3-t^2*x^3-t^2+t*x^3+t^3*x^3-t-t^3+x*t^3-t*x^2-x^2*t^3;
...
Monic((t-1)*x-t,p,q);
# {--> enter Monic, args = (-1+t)*x-t, 1+t^2, 3




# <-- exit Monic (now at top level) = x+1-t}


Quo(-t-t^3, -1-t^2+t+t^3,t, 'rrr') mod q;
